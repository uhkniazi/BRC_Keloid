...
q = paste0('select File.*, Sample.idData from File, Sample 
           where (Sample.idData = 2) and (File.idSample = Sample.id) and (File.group1 like "S021") and (File.type = "fastq")')
...
cvQueries = paste0('select File.*, Sample.title from File, Sample 
           where (Sample.idData = 2 and Sample.id =', dfQuery$SampleID, ') and (File.idSample = Sample.id) and 
                   File.group1 like "S021" and (File.type = "fastq")')
## change this File.group1 to S032, S021 or S014 depending on sequencing run
...
oFile.param = file('AutoScripts/hisat2_S021_param.txt', 'wt')
...
oFile = file('AutoScripts/hisat2_S021.sh', 'wt')
...
number=$SGE_TASK_ID
paramfile=hisat2_S021_param.txt
