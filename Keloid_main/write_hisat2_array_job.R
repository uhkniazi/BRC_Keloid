...
q = paste0('select File.*, Sample.idData from File, Sample 
           where (Sample.idData = 2) and (File.idSample = Sample.id) and (File.group1 like "S032") and (File.type = "fastq")')
...
cvQueries = paste0('select File.*, Sample.title from File, Sample 
           where (Sample.idData = 2 and Sample.id =', dfQuery$SampleID, ') and (File.idSample = Sample.id) and 
                   File.group1 like "S032" and (File.type = "fastq")')
...
oFile.param = file('AutoScripts/hisat2_S032_param.txt', 'wt')
...
oFile = file('AutoScripts/hisat2_S032.sh', 'wt')
