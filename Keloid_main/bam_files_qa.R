...
setwd('Data_external/Aligned/S021/')
...
setwd(gcswd)
n = make.names(paste('CBamScaffold rd S021 spikein rds'))
lAllBams$meta = dfSample
lAllBams$desc = paste('CBamScaffold object from S021 sequencing run with spikein and quality 10 duplicates removed', date())
...
df = data.frame(idData=2, name=n, type='rds', location='~/Data/MetaData/', 
                comment='CBamScaffold object from S021 sequencing run with spikein quality 10 duplicates removed')
...
lAllBams$desc = NULL
lAllBams$meta = NULL
pdf(file='Keloid_main/Results/bam.q10.rd.qa.S021.pdf')
par(mfrow=c(2,2))
...
barplot(ivMat, las=2, main='Sequencing run S021 - Average Read Width', ylab = 'Average Read Width', cex.names =0.6)
...
barplot(iReadCount, las=2, main='Sequencing run S021 - Read Count', ylab = 'No. of Reads in Millions', cex.names =0.6)
...
boxplot(lMat, las=2, main='Sequencing run S021 - Average Binned Coverage', ylab = 'Average', outline=F, xaxt='n')
axis(1, at=1:length(lMat), labels = names(lMat), cex.axis=0.7, las=2)



## take the average of the coverage for grouping factors
## creating a cleaner single figure for coverage
mCov = lMat.ordered[['chr2']]
dim(mCov)
colnames(mCov)
# match title to colnames
i = match(dfSample$title, colnames(mCov))
dfSample = dfSample[i,]

fac1 = gsub('^(K|N).+', '\\1', dfSample$title)
fac2 = gsub('.+(1|2).+', '\\1', dfSample$title)
fac = factor(paste(fac1, fac2, sep='-'))

m = sapply(levels(fac), function(x){
  i = which(fac == x)
  mCov.sub = rowMeans(mCov[,i])
  return(mCov.sub)
})

m = apply(m, 2, function(y) lowess(y, f=2/10)$y )
iCol = rainbow(nlevels(fac))
matplot(m, type='l', main=paste('Lowess fit to coverage', 'chr2'), col=iCol, xlab='Bins', sub='Binned Coverage', ylab='Coverage', 
        lty=1, lwd=2)

legend('bottomright', legend = colnames(m), ncol=1, col = iCol, lty=1, cex=1, lwd=2)

dev.off(dev.cur())

# bf = BamFile('/run/user/1000/gvfs/sftp:host=10.202.64.28,user=k1625253/users/k1625253/Data/ProjectsData/BRC_Keloid/Aligned/S021/K2-2nd_S021_R1_.fastq.gz_q10_sort_rd.bam')
# si = seqinfo(bf)
# si = as.data.frame(si)
