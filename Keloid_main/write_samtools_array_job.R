# File: write_samtools_array_job.R
...
q = paste0('select File.*, Sample.idData from File, Sample 
           where (Sample.idData = 2) and (File.idSample = Sample.id) and (File.group1 like "S014") and (File.type = "fastq")')
...
cvQueries = paste0('select File.*, Sample.title from File, Sample 
           where (Sample.idData = 2 and Sample.id =', dfQuery$SampleID, ') and (File.idSample = Sample.id) and 
                   File.group1 like "S014" and (File.type = "fastq")')
...
cvWorkingDir = '#$ -cwd'
cvJobName = '#$ -N samtools-array-S014'
cvStdout = '#$ -j y'
...
oFile.param = file('AutoScripts/samtools_S014_param.txt', 'wt')
...
oFile = file('AutoScripts/samtools_S014.sh', 'wt')
...
number=$SGE_TASK_ID
paramfile=samtools_S014_param.txt
