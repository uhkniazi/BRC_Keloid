...
lComparison = apply(mDat, 1, f_lpostMean, fSamples, sim.size = 1000)
# for each member of list perform comparison
lPvalue = lapply(lComparison, function(x){
  df = data.frame(do.call(cbind, x))
  st = stack(df)
  return(get.pairwise.prob(st$values, st$ind, p.adjust.method = 'BH'))
})
...
cvCommonGenes = names(lPvalue.sig)
...
keloid_keratinocyte_data = list(graph=oGr, matrix=mCounts, groups=fGroups)
save(keloid_keratinocyte_data, file='Objects/keloid_keratinocyte_data.rds')

library('RMySQL')
db = dbConnect(MySQL(), user='rstudio', password='12345', dbname='Projects', host='127.0.0.1')
dbListTables(db)
dbListFields(db, 'MetaFile')
df = data.frame(idData=4, name='keloid_keratinocyte_data.rds', type='rds', 
                location='~/Data/MetaData/', comment='GSE44270 graph object, expression matrix and grouping factor for keratinocytes')
